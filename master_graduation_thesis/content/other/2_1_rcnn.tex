\def\rcnn{
    ...
    Tuy nhiên, ở thời điểm mà mô hình R-CNN ra đời, do các mô hình học sâu chưa thật sự phát triển nên mô hình R-CNN không hoàn toàn sử dụng học sâu mà vẫn dựa trên kết quả của thuật toán xử lý ảnh như Selective Search \cite{uijlings2013selective}.
    
    \subsubsection*{Kiến trúc mô hình R-CNN}
    Là một trong số các mô hình two-stage, mô hình R-CNN bao gồm hai thành phần: \\
    - Phần region proposals module \index{region proposals module} mà mô hình R-CNN sử dụng là thuật toán Selective Search ở trên.
    ...
    - Phần feature extraction module \index{feature extraction module} của mô hình R-CNN là một mô hình phân lớp ảnh, cụ thể theo \cite{girshick2014rich} là AlexNet.
    ...
    \noindent
    Đối với thành phần feature extraction module \index{feature extraction module}, nhóm tác giả thực hiện hai thí nghiệm: \\
    \textit{Thí nghiệm 1: Finetune mô hình phân lớp CNN} \\
    Nhóm tác giả thay thế lớp fully-connected cuối cùng của mô hình AlexNet (gồm 1000 lớp theo bộ dữ liệu ImageNet) bằng một lớp fully-connected mới (gồm 21 lớp: 20 lớp của bộ dữ liệu VOC và 1 lớp ứng với background \index{background}).
    ...
    Những khu vực có chỉ số IoU \index{IoU} với groundtruth \index{groundtruth} bounding box \index{bounding box} >= 0.5 được gán nhãn là lớp của bounding box \index{bounding box} đó.
    Và ngược lại, sẽ được gán nhãn là lớp background \index{background}. \\
    TODO Appendix B \\
    ...
    \noindent
    \textit{Thí nghiệm 2: Xây dựng bộ phân lớp đối tượng \index{lớp đối tượng} bằng SVM} \\
    ...
    Mỗi mô hình SVM tương ứng với một lớp đối tượng \index{lớp đối tượng} và mỗi mô hình SVM trả lời cho câu hỏi, đặc trưng của khu vực đó có phải là đối tượng đó hay không.
    Những đặc trưng của khu vực có chỉ số IoU \index{IoU} với groundtruth \index{groundtruth} bounding box \index{bounding box} >= 0.3 sẽ được gán nhãn là lớp của bounding box \index{bounding box} đó.
    Và ngược lại, những đặc trưng của khu vực có chỉ số IoU \index{IoU} với groundtruth \index{groundtruth} bounding box \index{bounding box} < 0.3 sẽ được gán nhãn không phải là lớp của bounding box \index{bounding box} đó. \\
    TODO Appendix B \\

    \textit{Bước 3 (Tuỳ chọn)}: Bổ sung thêm hàm loss cải thiện khả năng định vị khu vực \\
    TODO Appendix C \\

    \subsubsection*{Kết quả của mô hình R-CNN}
    ...
    Với cấu hình cơ bản, R-CNN đạt được chỉ số average precision cao hơn trên tất cả các lớp đối tượng \index{lớp đối tượng} và chỉ số mAP cao hơn 10\% so với các mô hình hiện đại nhất tại thời điểm đó.
    Hơn nữa, với cấu hình R-CNN BB, mô hình còn đạt được chỉ số AP trên tất cả các lớp đối tượng \index{lớp đối tượng} và chỉ số mAP cao hơn 3\% so với cấu hình cơ bản R-CNN.
    \begin{figure}[H]
        \centering
    ...
    \end{figure}
    \noindent
    Cuối cùng, nhóm tác giả so sánh vai trò của feature extraction module \index{feature extraction module} đối với kết quả chung của mô hình R-CNN.
    Trong hình \ref{fig:rcnn_results_2}, nhóm tác giả xét đến hai mô hình có thể sử dụng trong feature extraction module \index{feature extraction module}: \\
    ...
    \subsubsection*{Vấn đề tồn đọng của mô hình R-CNN}
