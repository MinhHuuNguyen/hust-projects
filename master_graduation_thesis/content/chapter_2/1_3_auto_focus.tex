\def\autofocus{
    ...
    Sau khi mô hình đã được train và predict ra được pixels \index{pixels} cần được focus, mô hình cần một thuật toán để crop ra được khu vực cần focus trên ảnh làm đầu vào cho mô hình AutoFocus \index{AutoFocus} lượt tiếp theo.
    ...
    \noindent
    Trong quá trình predict, sau khi mô hình đã predict được các pixels \index{pixels} cần được focus (ký hiệu là $\mathcal{P}$) trên Focus Pixel  mask, ta biến đổi mask này trở về dạng binary mask bằng threshold $t$.
    ...
    Một vấn đề cần phải giải quyết khi thực hiện predict với ý tưởng Image Pyramids trong bài toán nhận diện đối tượng \index{nhận diện đối tượng} là việc tổng hợp lại các bounding box \index{bounding box}.
