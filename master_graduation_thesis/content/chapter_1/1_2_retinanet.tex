\def\retinanet{
    ...
    \noindent
    \textbf{\textit{Tổng quan về các mô hình nhận diện đối tượng \index{nhận diện đối tượng} một pha \index{một pha}}} \\
    ...
    \noindent
    \textbf{\textit{Hàm Focal loss \index{Focal loss}}} \\
    ...
    Chi tiết về sức mạnh của FPN \index{FPN} đã được thảo luận ở \textit{phần 2.2. Kiến trúc Feature Pyramid Networks}.
    - Phần trích xuất anchor \index{anchor} được thực hiện tương tự với cách trích xuất của mô hình RPN biến thể đã phân tích ở \textit{phần 2.2}.
    ...
    Trong thử nghiệm của RetinaNet \index{RetinaNet} với các tham số khác nhau của hàm Focal loss \index{Focal loss}, tất cả các cấu hình được train với bộ dữ liệu \textit{COCO trainval135k} và kết quả thu được đánh giá trên bộ dữ liệu \textit{COCO minival}.
