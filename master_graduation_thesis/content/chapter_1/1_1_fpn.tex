\def\fpn{
    Các kiến trúc backbone như AlexNet \cite{krizhevsky2012imagenet}, VGG \cite{simonyan2014very}, InceptionNet \cite{szegedy2015going}, SqueezeNet \cite{iandola2016squeezenet} và đặc biệt là ResNet \cite{he2016deep} đã đạt những thành công nhất định.
    Tuy nhiên, các kiến trúc backbone trên vẫn gặp phải một vấn đề về chênh lệch kích thước giữa các đối tượng trong ảnh.
    Feature Pyramid Networks \cite{lin2017feature} (gọi tắt là FPN) được giới thiệu như một kiến trúc backbone nhằm giải quyết vấn đề trên.
    Việc sử dụng FPN như là kiến trúc backbone kết hợp cùng mô hình Faster R-CNN đã vượt qua rất nhiều các mô hình nhận diện đối tượng khác để trở thành mô hình SOTA ở thời điểm đó.
